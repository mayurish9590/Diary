...
class EmojiPopup: UIView{
   ...
   // let emoji : [String] = ["Smile", "Party", "Sleepy", "Angry","Funny", "Crying"]
    ...
    {
        EmojiPopup.instance.emojiCollectionView.delegate = self
         EmojiPopup.instance.emojiCollectionView.dataSource = self
         EmojiPopup.instance.emojiCollectionView.register(UINib(nibName: "EmojiCollectionViewCell", bundle: nil), forCellWithReuseIdentifier: "EmojiCell")
        
    }
      
 func getView() -> UIView!
    {
        if let nib = Bundle.main.loadNibNamed(Nib.emojiPopup, owner:  EmojiPopup.instance, options: nil),
                   let nibView = nib.first as? UIView{
                   nibView.autoresizingMask = [.flexibleWidth, .flexibleHeight]
            nibView.frame =  CGRect(x:UIScreen.main.bounds.width / 3, y: UIScreen.main.bounds.height / 3, width: UIScreen.main.bounds.width / 1.6 , height: UIScreen.main.bounds.height / 4.8)
